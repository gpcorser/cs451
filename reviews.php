 ...
 * - Non-admins with assignmentFilter > 0: only teammates for that assignment (excluding self)
 * - Fallback: empty list if no team found
 */
...
if (!$isAdmin && $assignmentFilter > 0) {
    ...
    $stmt = $pdo->prepare('
        ...
        LIMIT 1
    ');
    $stmt->execute([':aid' => $assignmentFilter, ':pid' => $loggedInUserId]);
    ...
    if ($teamRow) {
        ...
        $stmt2 = $pdo->prepare('
            ...
            ORDER BY p.lname ASC, p.fname ASC
        ');
        $stmt2->execute([':aid' => $assignmentFilter, ':tn' => $teamNumber]);
...
// Modal open logic:
// - Open automatically when editing (via ?edit=...)
// - Also open if user just posted and got a validation message while staying on same page (optional)
// We'll open on editingReview only (predictable).
$autoOpenModal = !empty($editingReview);
...
<a class="btn btn-outline-modern btn-sm"
   ...
   PDF <?php echo (int)$pf['file_index']; ?>
    ...
    <!-- ACTION ROW: Button opens modal -->
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div class="helper-text">
            Reviewer: <strong><?php echo htmlspecialchars($userEmail); ?></strong> (ID <?php echo $loggedInUserId; ?>)
            <?php if ($assignmentFilter > 0): ?>
                <span class="ms-2 text-muted">Filter: Assignment ID <?php echo (int)$assignmentFilter; ?></span>
            <?php endif; ?>
        </div>
        ...
        <div class="d-flex gap-2">
            ...
            <?php if ($editingReview): ?>
                <a class="btn btn-outline-modern btn-sm"
                   href="reviews.php<?php echo $assignmentFilter > 0 ? '?assignment_id=' . (int)$assignmentFilter : ''; ?>">
                    Cancel Edit
                </a>
            <?php endif; ?>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="reviews.php?edit=<?php echo (int)$r['id']; ?><?php
                                    if ($assignmentFilter > 0) echo '&assignment_id=' . (int)$assignmentFilter;
                                ...
                                <?php if ($r['date_finalized'] === null): ?>
                                    <form method="post"
                                          action="reviews.php<?php echo $assignmentFilter > 0 ? '?assignment_id=' . (int)$assignmentFilter : ''; ?>"
                                          class="d-inline">
    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="reviewModalLabel"><?php echo htmlspecialchars($formTitle); ?></h5>
            ...
            <form method="post" action="reviews.php<?php
                $qs = [];
                if ($assignmentFilter > 0) {
                    $qs[] = 'assignment_id=' . (int)$assignmentFilter;
                }
                if ($editingReview) {
                    $qs[] = 'edit=' . (int)$valId;
                }
            ...
            ?>">
                <div class="modal-body">
                        ...
                        <div class="col-md-5">
                            ...
                            <select name="student_id" id="student_id" class="form-control form-control-sm" required>
                                ...
                                <?php foreach ($reviewablePersons as $p): ?>
                                    ...
                                    ?>
                                    <option value="<?php echo $pid; ?>" <?php if ($pid === (int)$valStudentId) echo 'selected'; ?>>
                        ...
                        <div class="col-md-5">
                            ...
                            <select name="assignment_id" id="assignment_id" class="form-control form-control-sm" required>
                                ...
                                <?php foreach ($assignments as $a): ?>
                                    <?php $aid = (int)$a['id']; ?>
                                    <option value="<?php echo $aid; ?>" <?php if ($aid === (int)$valAssignmentId) echo 'selected'; ?>>
                    ...
                    <div class="small text-muted mt-2">
                        Reviewer: <?php echo htmlspecialchars($userEmail); ?> (ID <?php echo $loggedInUserId; ?>)
                        <?php if ($editingReview): ?>
                            <span class="ms-2">Editing Review ID <?php echo (int)$valId; ?></span>
                        <?php endif; ?>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-outline-modern btn-sm" data-bs-dismiss="modal">Close</button>
                    <button type="submit" class="btn btn-modern btn-sm">
                        <?php echo $editingReview ? 'Update Review' : 'Submit Review'; ?>
                    </button>
                </div>
            </form>
...
<script>
(function() {
    const autoOpen = <?php echo $autoOpenModal ? 'true' : 'false'; ?>;
    if (autoOpen) {
